...
export const handlePostgres = async () => {
  if (!config.backup.postgres || Object.keys(config.backup.postgres).length === 0) {
  ...
  for (const [key, config] of containers) {
    ...
    try {
      if (!options.dryRun) {
        ...
        if (!isRunning) {
          log.info(`⚠️  Container ${config.container} is not running, skipping...`);
          continue;
        }
    ...
    const SQL_FILE = join(TEMP_DIR, `${key}_${config.container}_${config.database}.sql`);
